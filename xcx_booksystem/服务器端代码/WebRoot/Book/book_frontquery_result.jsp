...
<jsp:include page="../header.jsp"></jsp:include>
	<div class="col-md-9 wow fadeInLeft">
		<ul class="breadcrumb">
  			...
  			<a class="pull-right" href="<%=basePath %>Book/book_frontAdd.jsp" style="display:none;">添加图书</a>
		</ul>
		<div class="row">
			<%
				/*计算起始序号*/
			...
			<div class="col-md-3 bottom15" <%=clearLeft %>>
			  <a  href="<%=basePath  %>Book/<%=book.getBarcode() %>/frontshow"><img class="img-responsive" src="<%=basePath%><%=book.getBookPhoto()%>" /></a>
			     <div class="showFields">
			     	...
			     	<div class="field">
	            		图书文件:<%=book.getBookFile().equals("")?"暂无文件":"<a href='" + basePath + book.getBookFile() + "' target='_blank'>" + book.getBookFile() + "</a>"%>
			     	</div>
			        <a class="btn btn-primary top5" href="<%=basePath %>Book/<%=book.getBarcode() %>/frontshow">详情</a>
			        <a class="btn btn-primary top5" onclick="bookEdit('<%=book.getBarcode() %>');" style="display:none;">修改</a>
			        <a class="btn btn-primary top5" onclick="bookDelete('<%=book.getBarcode() %>');" style="display:none;">删除</a>
			     </div>
			</div>
			<%  } %>
	...
	<div class="col-md-3 wow fadeInRight">
		...
		<form name="bookQueryForm" id="bookQueryForm" action="<%=basePath %>Book/frontlist" class="mar_t15" method="post">
